...
5. **Checks the game is running** — if no window matches `--window-title`, it
   ...
   The check (like every game-window decision — capture, touch injection,
   resizing) binds to the **chosen install only**: a matching window counts
   only when its process runs from the chosen game's folder, so **other WoW
   installs may keep running at the same time** — they are simply ignored,
   and one being open never makes the wizard wait for "WoW" to close while
   editing the chosen install's `Config.wtf`. (If the game runs elevated and
   Windows refuses the process query, the check falls back to matching by
   title alone and says so in the log.)
...
3. **Add to Home Screen** for the real fullscreen experience:
   ...
   Open the pairing link in the browser first, then install: the page binds
   the install to your pairing token (the installed app's launch URL carries
   it), so the home-screen app opens **pre-paired** even though installed
   apps get their own separate storage (iOS especially).
...
3. Add these lines — or replace them if a `SET gx...` line already exists:
   ...
   SET gxWindow "1"
   SET gxMaximize "0"
   SET gxWindowedResolution "552x984"
   SET checkAddonVersion "0"
   ```

   (`checkAddonVersion "0"` = "Load out of date AddOns": without it a game
   patch silently disables the addon.)
...
play on the phone; the PC window just needs to exist, stay visible, not be
minimized, and **fit on the monitor** — a window taller than the screen cannot
be captured (the off-screen rows do not exist on the desktop), which is
exactly why `fit` is the default.
...
- **Typing** (chat, auction search, mail recipient/subject, add-friend) goes
  ...
  phone deck's **Aa** key (bottom of the screen, below the video).
...
| A **different WoW install is running** at the same time — does it interfere? Wizard once said "WoW is running" / waited for it to close although the chosen game was closed | It may run freely: every game-window decision (the running check, the `Config.wtf` write guard, capture, touch injection, resizing) accepts a window only when its **process runs from the chosen install's folder**, so unrelated instances are ignored (pre-v0.4.3 versions matched by window title alone — that's the old behavior you may remember). Two caveats: if the *chosen* game runs **elevated** ("Run as administrator") Windows can refuse the process check and matching falls back to title-only (a log line says so — avoid running the unrelated WoW then); and on the non-NVENC **gdigrab** capture path, two windows with the *identical* full title can still confuse ffmpeg's title-based grabber — the dashboard warns `the stream may show the WRONG WoW instance` while that's the case; close the other same-titled window (the NVENC/ddagrab default pins capture by screen position and is immune). |
...
| `no visible window with "World of Warcraft" in its title` at startup | WoW isn't running, is minimized, or has a different title. Start WoW first (windowed, not minimized) or let wizard step 5 launch it. If the title truly differs, pass the actual text via `--window-title`. The variant `found N visible window(s) … but none belongs to the configured install` means the WoW that IS running is a *different install* than the chosen one — start the chosen game, or restart with `--choose-game`/`--game-exe` to switch. |
...
| Server log note: **"gxResolution is stale: Config.wtf says … but the live client area is …"** | Band layout (1.12), informational only — nothing is wrong with the stream. The `gxResolution` CVar (typically your desktop resolution, wizard-written) disagrees with the live window's aspect (typical: the window is maximized above the taskbar, whose client area is shorter). An up-to-date WowMobile addon detects exactly this and lays its band out from the **live** window (`/wm status` shows basis `gx-derived`), so the addon UI and the stream's crop still line up pixel-perfect; running maximized on a 4K desktop is a perfectly fine way to play and the note never clears while you do — ignore it. It matters only if the addon predates the band layout (pre-v0.4.x): then the UI sits cut/shifted at the band edges — re-run the wizard (it installs the current addon) and `/reload`. To refresh the CVar itself instead: close WoW fully, re-run the wizard, un-maximize, restart WoW. |
