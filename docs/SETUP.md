...
Windows 10+ gaming PC, WoW Classic Era, FFmpeg, Go 1.24+, a phone on the same
Wi-Fi (5 GHz strongly recommended).
...
(or download a full build from [gyan.dev](https://www.gyan.dev/ffmpeg/builds/)
and add its `bin` folder to your `PATH`). Verify from a **new** terminal:
...
With [Go 1.24+](https://go.dev/dl/) installed, from the repo root on the
gaming PC:

```
cd server
go build -o ..\wowstreamd.exe .\cmd\wowstreamd
cd ..
.\wowstreamd.exe
```

Run it **from the repo root** so it finds the phone client files (it serves
`./client`, falling back to `../client`; override with `--client-dir`). Start
WoW *before* the server — it locates the game window at startup and exits with
guidance if none is found.

To cross-compile from macOS/Linux instead:
`cd server && GOOS=windows GOARCH=amd64 go build -o wowstreamd.exe ./cmd/wowstreamd`,
then copy the `.exe` next to the repo's `client/` directory on the PC.
...
| `--client-dir` | `./client`, then `../client` | Directory with the phone client PWA |
...
Allow it through Windows Defender Firewall (**private networks**) when
prompted, or the phone can't reach port 8443.
...
| `no visible window with "World of Warcraft" in its title` at startup | WoW isn't running, is minimized, or has a different title. Start WoW first (windowed, not minimized). If the title truly differs, pass the actual text via `--window-title`. |
...
| Addon not loading / stock UI shows | The folder must be exactly `Interface\AddOns\WowMobile` with `WowMobile.toc` directly inside (no doubled `WowMobile\WowMobile` nesting). Enable it at character select; if flagged out-of-date after a game patch, check **Load out of date AddOns**. |
| UI misaligned, taps land in the wrong place | `gxWindowedResolution` and `--resolution` disagree, or the window got resized. They must match exactly; re-check step 1 and restart both WoW and the server. |
| Taps near the world/deck boundary move the character or drag the camera | The addon's `/wm viewport` and the phone's **World viewport** setting disagree (both default 1080). Set them to the same number — step 6. |
